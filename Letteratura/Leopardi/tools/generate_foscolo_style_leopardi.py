...
def paragraphs_to_html(content):
...
def video_html(video, root):
    ...
    if local:
        pieces.append(f'<video controls preload="metadata" src="{root}{e(local)}"></video>')
    if embed:
        pieces.append(f'<iframe src="{e(embed)}" title="{e(video["title"])}" allowfullscreen></iframe>')
    if online:
        pieces.append(f'<p class="video-link-row"><a class="button-link" href="{e(online)}" target="_blank" rel="noopener">Apri video online</a></p>')
...
def write_lesson(item):
    ...
    article = f"""
      <article class="lesson-article">
        ...
        {paragraphs_to_html(item["content"])}
      </article>
"""
    body = f"""
    ...
    cards = "\n".join(
        f"""
          <a class="section-card" href="{item_link(item)}">
            ...
            <span>{e(item["kicker"])} · {len(item["videos"])} video · test incluso</span>
          </a>
"""
        for item in items
    )
...
def write_video(data):
    ...
    for item in data["items"]:
        for video in item["videos"]:
            ...
            media = ""
            if local:
                media += f'<video class="video-frame" controls preload="metadata" src="{e(local)}"></video>'
            if embed:
...
"""
    (ROOT / "video.html").write_text(page_shell("Video", body, page="video", current="video.html", footer_parts=("Video associati", "Locale e online", "Materiale integrato")), encoding="utf-8")
...
def write_service_worker(data):
    ...
    sw = f"""const CACHE_NAME = "leopardi-foscolo-style-v1";
