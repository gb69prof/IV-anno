...
def load_data() -> dict:
    code = "global.window={};require('./js/data.js');process.stdout.write(JSON.stringify(window.REST_DATA));"
...
class PdfDoc(BaseDocTemplate):
    def __init__(self, filename: Path, pagesize=A4, title="La Restaurazione"):
        super().__init__(
...
def draw_page(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, 0.83 * cm, "La Restaurazione · gbprof e Libera")
...
def make_lesson(lesson: dict):
    ...
    doc = PdfDoc(path, title=f"{lesson['title']} - La Restaurazione")
...
def make_sources():
    ...
    doc = PdfDoc(path, title="Le fonti in dialogo - La Restaurazione")
    story = cover("Le fonti in dialogo", "Promesse, autorità e compromessi", "Laboratorio storico", DATA["meta"]["question"])
    for index, source in enumerate(DATA["sources"]):
        story += [Paragraph(source["title"], STYLES["H1Rest"]), Paragraph(f"{source['type']} · {source['date']} · {source['provenance']}", STYLES["SmallRest"])]
        story += [Paragraph(f"«{quote}»", STYLES["QuoteRest"]) for quote in source["excerpt"]]
        rows = [
            ["Destinatario", source["audience"]], ["Scopo", source["purpose"]], ["Limiti", source["limits"]], ["Domanda", source["question"]]
        ]
...
def make_maps():
    ...
    doc = PdfDoc(path, pagesize=landscape(A4), title="Mappe e schemi - La Restaurazione")
    story = cover("Mappe e schemi", "Sette visualizzazioni per studiare i rapporti", "La Restaurazione")
    schemes = [
        ("1. Il mondo precedente", ["Monarchia costituzionale: potere limitato", "Repubblica radicale: sovranità popolare", "Repubblica censitaria: diritti politici legati al censo", "Bonapartismo: capo, esercito, plebiscito e Stato moderno"]),
        ("2. La frattura", ["Lipsia 1813: sconfitta militare", "Chaumont 1814: coalizione politica", "Parigi 1814: Francia contenuta ma non distrutta", "Vienna e Waterloo 1815: nuovo ordine e fine dell'Impero"]),
        ("3. I pilastri di Vienna", ["Legittimità: dinastie", "Equilibrio: nessuna egemonia", "Concerto: consultazione fra potenze"]),
        ("6. Forza e limiti", ["Forza: pace, equilibrio, reintegrazione della Francia", "Limiti: censura, suffragio ristretto, nazionalità ignorate"]),
        ("7. Liberali e conservatori", ["Conservatori: storia, autorità, continuità, gradualità", "Liberali: diritti, legalità, Costituzione, rappresentanza"]),
    ]
    for title, nodes in schemes[:3]:
        story += [Paragraph(title, STYLES["H1Rest"]), Table([[Paragraph(node, STYLES["SmallRest"]) for node in nodes]], colWidths=[24.2 * cm / len(nodes)] * len(nodes), style=TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#f0dfbd")), ("GRID", (0, 0), (-1, -1), 1, GOLD),
            ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("PADDING", (0, 0), (-1, -1), 14),
        ])), Spacer(1, 0.5 * cm)]
    story.append(PageBreak())
    for title, filename in [("4. L'Europa nel 1815", "europa-1815.png"), ("5. L'Italia nel 1815", "italia-1815.png")]:
        story += [Paragraph(title, STYLES["H1Rest"]), Image(str(ROOT / "assets" / "img" / "maps" / filename), width=22.5 * cm, height=14.5 * cm, kind="proportional"), PageBreak()]
    for title, nodes in schemes[3:]:
        story += [Paragraph(title, STYLES["H1Rest"]), bullet_table(nodes), Spacer(1, 1 * cm)]
    doc.build(story)
