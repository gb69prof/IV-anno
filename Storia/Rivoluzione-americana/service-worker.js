...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET'||new URL(req.url).origin!==self.location.origin)return;const own=req.url.startsWith(new URL('./',self.location.href).href);if(!own&&!URLS.has(req.url))return;
 ...
 try{const response=await fetch(req);if(response.ok&&(URLS.has(req.url)||own)){await cache.put(req,response.clone())}if(!response.ok&&cached)return cached;return response}catch{if(cached)return cached;if(req.mode==='navigate'&&own)return (await cache.match(new URL('index.html',self.location.href).href))||Response.error();return Response.error()}})());
});
