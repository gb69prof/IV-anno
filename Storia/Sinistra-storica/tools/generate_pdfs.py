#!/usr/bin/env python3
"""Genera otto lezioni, dossier fonti e dossier cartografico da js/data.js."""
...
DEEP=colors.HexColor('#352f4a'); ACCENT=colors.HexColor('#a24f5f'); GOLD=colors.HexColor('#c49a55'); PAPER=colors.HexColor('#fffaf4'); INK=colors.HexColor('#302b35'); MUTED=colors.HexColor('#6f6874')
...
S.add(ParagraphStyle(name='TableSmall',parent=S['BodyText'],fontName='UnitSans',fontSize=7.9,leading=10.3,textColor=MUTED,spaceAfter=0))
...
def maps_pdf():
    ...
    with tempfile.TemporaryDirectory(prefix='sinistra-mappe-') as temp:
        ...
        doc.build(story)
def synthesis_pdf():
    doc=Doc(OUTA/'sintesi-generale.pdf','Sintesi generale');story=cover('Sintesi generale','Risultati, costi e problemi aperti','1876-1887',D['meta']['question'])
    story += [Paragraph('La tensione fondamentale',S['H1x']),Paragraph(D['meta']['thesis'],S['B']),Paragraph('Otto problemi',S['H1x'])]
    rows=[[Paragraph(f"<b>{l['number']}. {l['title']}</b>",S['TableSmall']),Paragraph(l['answer'],S['TableSmall'])] for l in D['lessons']]
    story.append(Table(rows,colWidths=[5.1*cm,10.2*cm],repeatRows=0,style=TableStyle([('GRID',(0,0),(-1,-1),.4,colors.HexColor('#c8b78f')),('BACKGROUND',(0,0),(0,-1),colors.HexColor('#eee1c9')),('VALIGN',(0,0),(-1,-1),'TOP'),('PADDING',(0,0),(-1,-1),4.5)])))
    story += [PageBreak(),Paragraph('Cronologia ragionata',S['H1x'])]
    timeline=[[Paragraph('<b>'+t['date']+'</b>',S['TableSmall']),Paragraph('<b>'+t['title']+'</b><br/>'+t['text'],S['TableSmall']),Paragraph(t['scale'],S['TableSmall'])] for t in D['timeline']]
    story.append(Table(timeline,colWidths=[2.8*cm,10*cm,2.5*cm],repeatRows=0,style=TableStyle([('LINEBELOW',(0,0),(-1,-1),.35,colors.HexColor('#c8b78f')),('VALIGN',(0,0),(-1,-1),'TOP'),('PADDING',(0,0),(-1,-1),3.5)])))
    ...
    story += essentials
    story += [PageBreak(),Paragraph('Tesi finale',S['H1x']),Paragraph(D['meta']['question'],S['Q']),Paragraph('Formula una risposta che utilizzi almeno tre lezioni, due fonti, una visualizzazione e una possibile obiezione. Distingui ciò che nel 1876 appariva necessario da ciò che dipese da una scelta politica.',S['B']),Spacer(1,10*cm)]
    doc.build(story)
def main():
