(() => {
  ...
  const readCompleted = () => {
    try {
      const value = JSON.parse(localStorage.getItem(`${D.meta.storageKey}:completed`) || "[]");
  ...
  document.getElementById("homePeople").innerHTML = D.people.slice(0, 5).map(p => `<a class="portrait-card" href="app.html#biografia/${p.id}"><img src="${p.image}" alt="Ritratto di ${p.name}" loading="lazy"><span><strong>${p.name}</strong><small>${p.role}</small></span></a>`).join("");
