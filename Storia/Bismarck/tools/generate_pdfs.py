...
DEEP=colors.HexColor('#173747'); ACCENT=colors.HexColor('#9d4938'); GOLD=colors.HexColor('#c69b4b'); PAPER=colors.HexColor('#fffaf1'); INK=colors.HexColor('#252a2d'); MUTED=colors.HexColor('#687075')
...
def maps_pdf():
    ...
    with tempfile.TemporaryDirectory(prefix='bismarck-mappe-') as temp:
        ...
        doc.build(story)
def synthesis_pdf():
    doc=Doc(OUTA/'sintesi-generale.pdf','Sintesi generale');story=cover('Sintesi generale','Risultati, costi e problemi aperti','1815-1890',D['meta']['question'])
    ...
    story += essentials
    story += [PageBreak(),Paragraph('Tesi finale',S['H1x']),Paragraph(D['meta']['question'],S['Q']),Paragraph('Formula una risposta che utilizzi almeno tre lezioni, due fonti, una visualizzazione e una possibile obiezione. Distingui condizioni strutturali, scelte politiche e conseguenze non intenzionali.',S['B']),Spacer(1,10*cm)]
    doc.build(story)
def main():
