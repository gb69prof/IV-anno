...
DEEP=colors.HexColor('#172c33'); ACCENT=colors.HexColor('#9c4d3f'); GOLD=colors.HexColor('#c69b4c'); PAPER=colors.HexColor('#fffaf0'); INK=colors.HexColor('#202725'); MUTED=colors.HexColor('#68706b')
def load():
...
S.add(ParagraphStyle(name='K',parent=S['Normal'],fontName='Helvetica-Bold',fontSize=9,leading=12,textColor=GOLD,alignment=TA_CENTER,spaceAfter=10))
S.add(ParagraphStyle(name='T',parent=S['Title'],fontName='Times-Bold',fontSize=31,leading=34,textColor=DEEP,alignment=TA_CENTER,spaceAfter=10))
S.add(ParagraphStyle(name='Q',parent=S['BodyText'],fontName='Times-Italic',fontSize=13,leading=19,textColor=ACCENT,alignment=TA_CENTER,spaceAfter=14))
S.add(ParagraphStyle(name='H1x',parent=S['Heading1'],fontName='Times-Bold',fontSize=22,leading=25,textColor=DEEP,spaceBefore=10,spaceAfter=9))
S.add(ParagraphStyle(name='H2x',parent=S['Heading2'],fontName='Times-Bold',fontSize=16,leading=20,textColor=ACCENT,spaceBefore=13,spaceAfter=6))
S.add(ParagraphStyle(name='B',parent=S['BodyText'],fontName='Times-Roman',fontSize=10.8,leading=16.4,textColor=INK,spaceAfter=8))
S.add(ParagraphStyle(name='Small',parent=S['BodyText'],fontName='Helvetica',fontSize=8.6,leading=12.2,textColor=MUTED,spaceAfter=4))
S.add(ParagraphStyle(name='Sum',parent=S['BodyText'],fontName='Helvetica-Bold',fontSize=9.2,leading=13.5,textColor=INK,leftIndent=10,rightIndent=10,borderColor=GOLD,borderWidth=.7,borderPadding=8,backColor=colors.HexColor('#f0e3c7'),spaceAfter=10))
S.add(ParagraphStyle(name='Quote',parent=S['BodyText'],fontName='Times-Italic',fontSize=12,leading=17,leftIndent=14,rightIndent=14,textColor=INK,spaceAfter=9))
...
def maps_pdf():
    ...
    for i,m in enumerate(D['maps']):
        ...
        story += [Paragraph(m['title'],S['H1x']),Paragraph(m['text'],S['B']),Image(str(visual),width=23.5*cm,height=14.2*cm,kind='proportional'),Paragraph(m['note'],S['Small'])]
